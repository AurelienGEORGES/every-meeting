...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValidPassword) {
      setError(
        'Le mot de passe doit contenir 8 caractères minimum dont une majuscule, un chiffre et un caractère spécial minimum',
      );
      return;
    }
    try {
    ...
    <div className='w-full'>
      <div className='flex flex-col md:flex-row items-center justify-center min-h-screen'>
        <div className='md:w-1/2 flex justify-center md:order-1 order-2 my-5'>
          <img
            src={teamMeeting7}
            alt='reflexion en groupe'
            className='w-11/12 img-fluid rounded-xl'
          />
        </div>
            ...
            <div>
              <div className='flex justify-center'>
                <h1 className='text-center w-3/4 text-md md:text-2xl text-nav mx-1 my-5'>
                  Tu peux t'inscrire pour avoir accès aux nombreuses possibilités qu'offre Every
                  Meeting
                </h1>
              </div>
            </div>
